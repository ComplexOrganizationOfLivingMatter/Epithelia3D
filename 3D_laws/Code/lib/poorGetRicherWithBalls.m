function poorGetRicherWithBalls (path2save,gain4c,gain5c,gain6c,gain7c,gain8c)
    ...
    H = notBoxPlot(x,g,'markMedian',true,'jitter', 0.20);
...
%                 text(nSideAp-1,numbers(nUniqSides),num2str(countN(nUniqSides)),'HorizontalAlignment','center')
    ...
    savefig(h,fullfile(path2save,['fig_PoorGetRicherSalGland_nonboxplot_Boxes' date]));
    print(h,fullfile(path2save,['fig_PoorGetRicherSalGland_nonboxplot_Boxes' date]),'-dtiff','-r300')
