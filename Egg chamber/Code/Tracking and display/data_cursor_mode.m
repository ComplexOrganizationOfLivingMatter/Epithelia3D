           ...
           if x1(end) > W && y1(end) < H
               ...
               if y2(end)< H && x2(end) < W 
                   ...
                   if   L_img_previous(y2(end),x2(end))~=0  
                        fprintf('Change %d to %d \n',mask(y1(end),x1(end)-W),L_img_previous(y2(end),x2(end)))
                        are_sure=input('if are you sure insert 1: ');
                        pause;
                        if are_sure==1
                            mask_L=bwlabel(mask);
                            mask(mask_L==mask_L(y1(end),x1(end)-W))=L_img_previous(y2(end),x2(end));
                        end
                        ...
                        fprintf('%d is the new cell %d\n',mask(y1(end),x1(end)-W),new_cell)
                        is_new=input('if is a new cell insert 1: ');
                        pause;
                        if is_new==1
                            mask_L=bwlabel(mask);
                            mask(mask_L==mask_L(y1(end),x1(end)-W))=new_cell;
                            new_cell=new_cell+1;
