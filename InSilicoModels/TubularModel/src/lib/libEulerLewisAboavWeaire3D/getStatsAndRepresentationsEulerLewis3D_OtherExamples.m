function getStatsAndRepresentationsEulerLewis3D(numNeighOfNeighPerSurface,numNeighOfNeighAccumPerSurface,numNeighPerSurface,numNeighAccumPerSurfaces,areaCellsPerSurface,volumePerSurface,path2save,surfRatios)
    ...
    for SR = 1:length(surfRatios)
        ...
        subplot(3,4,10)
        errorbar(nUniqueNeighBasal,mean(meanVolumePerSideBasal),std(meanVolumePerSideBasal),'-o','MarkerSize',5,'MarkerEdgeColor','black','MarkerFaceColor','green')
        ylim([0 max([meanVolumePerSideApical(:);meanVolumePerSideBasal(:);meanVolumePerSideBasalAccum(:)])+max([stdVolumePerSideApical(:);stdVolumePerSideBasal(:);stdVolumePerSideBasalAccum(:)])])
        ...
        subplot(3,4,11)
        subplot(1, 2, SR)
        disp('CAREEEEEEEEEEEEEEEEEEEE!');
        errorbar(nUniqueNeighBasalAccum,mean(meanVolumePerSideBasalAccum, 'omitnan'),std(meanVolumePerSideBasalAccum, 'omitnan'),'o','MarkerSize',5,'MarkerEdgeColor','black','MarkerFaceColor','blue')
        ylim([0 max([meanVolumePerSideApical(:);meanVolumePerSideBasal(:);meanVolumePerSideBasalAccum(:)])+max([stdVolumePerSideApical(:);stdVolumePerSideBasal(:);stdVolumePerSideBasalAccum(:)])])
        
        
        allVolumens = table2array(vertcat(volumePerSurface{:}));
        allTotalNeighbours = table2array(vertcat(numNeighAccumPerSurfaces{:}));
        %figure;
%         for numPoint = 1:size(allTotalNeighbours, 1)
%             hold on;
%             plot(allTotalNeighbours(numPoint, SR), allVolumens(numPoint, SR), '*');
%         end
        [p, error] = polyfit(allTotalNeighbours(:, SR), allVolumens(:, SR), 1);
        f = polyval(p,[min(allTotalNeighbours(:, SR))-0.5; unique(allTotalNeighbours(:, SR)); max(allTotalNeighbours(:, SR))+0.5]);
        hold on
        plot([min(allTotalNeighbours(:, SR))-0.5; unique(allTotalNeighbours(:, SR)); max(allTotalNeighbours(:, SR))+0.5]',f,'Color','blue','LineWidth',1)
...
%         
        ...
        xNoNaN = nUniqueNeighBasalAccum(~nanNames);
        yNoNaN = meanNeigNeigh(~nanNames);
        meanNeighOfNeighAccumPerSideBasalAccum_All{SR} = [xNoNaN'; yNoNaN];
