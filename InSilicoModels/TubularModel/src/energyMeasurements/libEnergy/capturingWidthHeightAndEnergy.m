function [H1Length,H2Length,W1Length,W2Length,sumEdgesOfEnergy,edgeLength,emptyIndex]=capturingWidthHeightAndEnergy(verticesPerCell,vertices,pairValidCellsPreserved,cellsInMotifNoContactValidCellsPreserved,W)
    ...
    emptyIndex=cell2mat(cellfun(@(x) isempty(x),verticesCell_1_2,'UniformOutput',false));   
    ...
    emptyIndex=cell2mat(cellfun(@(x,y,z,zz) (length(x)==2 & length(y)==2 & length(z)==2 & length(zz)==2),vertH1default,vertH2default,vertW1default,vertW2default,'UniformOutput',false));
    
    verticesCell_1_2(emptyIndex,:)=[];
    vertH1default(emptyIndex,:)=[];
    vertH2default(emptyIndex,:)=[];
    vertW1default(emptyIndex,:)=[];
    vertW2default(emptyIndex,:)=[];

    
    
    %delete only vertices that are alones
