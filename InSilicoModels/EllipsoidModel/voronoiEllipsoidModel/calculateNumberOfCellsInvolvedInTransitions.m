function [proportionOfScutoids,proportionOfCellsWinning,proportionOfCellsLossing,proportionOfCellsInNoTransitions,proportionWinningNeigh,proportionLossingNeigh,proportionTransitionsPerCell]=calculateNumberOfCellsInvolvedInTransitions(neighsInner,neighsOuter,validCells)
%Calculation of scutoids (using lossing or winngin of neighbours)
    ...
    %Data for histograms 
    proportionWinningNeigh=[sum(winningPerCell==0),sum(winningPerCell==1),sum(winningPerCell==2),sum(winningPerCell==3),sum(winningPerCell==4)...
    ...
    proportionLossingNeigh=[sum(lossingPerCell==0),sum(lossingPerCell==1),sum(lossingPerCell==2),sum(lossingPerCell==3),sum(lossingPerCell==4)...
    ...
    proportionTransitionsPerCell=[sum(motifsTransitionPerCell==0),sum(motifsTransitionPerCell==1),sum(motifsTransitionPerCell==2),sum(motifsTransitionPerCell==3),sum(motifsTransitionPerCell==4)...
