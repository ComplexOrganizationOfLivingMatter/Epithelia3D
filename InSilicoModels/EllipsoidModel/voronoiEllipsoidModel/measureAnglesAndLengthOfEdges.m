function [ basalDataTransition,basalDataNoTransition ] = measureAnglesAndLengthOfEdges( L_basal,neighsApical,neighsBasal,validCells)
    ...
    totalCellsDilated=cell(max(unique(uniquePairOfNeighBasal)),1);
    for nCell=1:max(unique(uniquePairOfNeighBasal))
        if ismember(nCell,unique(uniquePairOfNeighBasal))
            ...
            totalCellsDilated{nCell}=imdilate(mask,se);
        end
    end
    ...
    for i=1:size(uniquePairOfNeighBasal,1)
        if any(ismember(uniquePairOfNeighBasal(i,:),validCells))
            mask=bwlabel(totalCellsDilated{uniquePairOfNeighBasal(i,1)}.*totalCellsDilated{uniquePairOfNeighBasal(i,2)});
            edge1=regionprops(mask,'MajorAxisLength','Orientation');
            [basalData(i).edgeLength,indexMax]=max(vertcat(edge1.MajorAxisLength));
            basalData(i).edgeAngle=edge1(indexMax).Orientation;
        end
    end
