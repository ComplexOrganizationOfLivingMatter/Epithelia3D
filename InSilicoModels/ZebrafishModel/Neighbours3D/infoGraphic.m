...
for numLayer=1:2
   ...
   % percentage90Shared=0;
    ...
    for numCellShared=1:size(sharedPercentageWithOnlyNextLayer,1)
        ...
        %numNeighSize=size(sharedPercentageWithOnlyNextLayer{numCellShared,1},1);
        ...
        for numNeigh=1:size(sharedPercentageWithOnlyNextLayer{numCellShared,1},1)
            ...
            elseif actualP>50 && actualP<=75
                percentage75Shared=percentage75Shared+1;
            elseif actualP>75
...
%             end
%             
%             acumPercentageByNumNeighs{numLayer,1}(numNeighSize,1)=percentage10Shared;
...
%     nameGraphicPercentage=[outputDir 'SharedInfoPercentageBetween' sprintf('%d',numLayer) 'And' sprintf('%d',numLayer+1) 'sample' sprintf('%d',folderNumber) '.jpg'];
